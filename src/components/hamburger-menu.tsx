...
const HamburgerMenu: React.FC<HamburgerMenuProps> = ({ links }) => {
    ...
    const { activeSection, setActiveSection, setTime0fLastClick } = 
    ...
    return (
     ...
     flex-col items-end gap-2 ">
       ...
       backdrop-blur-[0.5rem] border border-slate-400
       dark:border-white border-opacity-40 shadow2xl rounded-full
            ...
            >
                {links.map((link, index ) => (
                    ...
                    >
                       ...
                       {"text-gray-950 bg-slate-200 Idark:text-gray-200 dark:bg-gray-700 rounded": activeSection === link.hash,"rounded-t-xl round": index === 0,"rounded-b-xl round": index === links.length - 1,}
                        )}
                        href={link.hash}
                        onClick={()=>{
                            setActiveSection(link.hash);
                            setTime0fLastClick(Date.now());
                        }}
                        >
                           {link.nameEng}
                        </NextLink>
